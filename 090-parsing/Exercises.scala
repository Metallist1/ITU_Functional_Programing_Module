...
trait Parsers[ParseError, Parser[+_]]:
  ...
  extension [A](p1: Parser[A]) 
    ...
    def * : Parser[List[A]] = 
      p1.many
    def *>[B](p2: => Parser[B]) =
      p1.slice.map2(p2)((_, b) => b)
    def <*(p2: => Parser[Any]) =
      p1.map2(p2.slice)((a, b) => a)
  end extension
  ...
  // Exercise 4
  // A better name would be: howManyA  Use many and map to implement
...
object Sliceable 
  ...
  def succeed[A](a: A): Parser[A] = loc =>
    Success(a,0)
  ...
  extension [A](p: Parser[A]) 
    def or(p2: => Parser[A]): Parser[A] = loc => p(loc) match
      case Failure(_,_) => p2(loc)
      case r => r
  // Exercise 10

  def regex(r: Regex): Parser[String] = (s: ParseState) =>
     r.findPrefixOf(s.loc.input.substring(s.loc.offset)) match
      case Some(a) => 
        if s.isSliced then Slice(a.length)
        else Success(a, a.length)
      case None => Failure(s.loc.toError(s"regex $r"), true)
...
class JSONParser[ParseError, Parser[+_]](P: Parsers[ParseError,Parser]):
  ...
  lazy val jnull: Parser[JSON] =
    regex("null".r) |* succeed(JNull)

  lazy val jbool: Parser[JSON] =
    (regex("true".r) |* succeed(JBool(true))) | (regex("false".r) |* succeed(JBool(false)))
  lazy val jstring: Parser[JString] = QUOTED.map(JString(_))

  lazy val jnumber: Parser[JNumber] = DOUBLE.map(JNumber(_))

  // Exercise 13


  private lazy val commaSeparatedVals: Parser[List[JSON]] =
    (ws.? |* json *| ws.?).map2(((ws.? ** char(',') ** ws.?) |* json *| ws.?).many)((head, tail) => head::tail) | ws.?.map(head => List.empty)

  lazy val jarray: Parser[JArray] =
    {char('[') |* ws.? |* commaSeparatedVals *| char(']') *| ws.?}.map {l => JArray(l.toVector)}

  lazy val field: Parser[(String,JSON)] =
    (ws.? |* QUOTED *| ws.?) ** ((char(':') ** ws.?) |* json *| ws.?)

  private lazy val commaSeparatedFields: Parser[List[(String,JSON)]] =
    (field ** (char(',') *| ws.? |* field).*).map((head, tail) => head::tail)

  lazy val jobject: Parser[JObject] =
    (char('{') |* commaSeparatedFields *| char('}')).map(fields => JObject(fields.toMap))

  lazy val json: Parser[JSON] =
    ws.? |* {jstring | jnull | jbool | jobject | jarray |jnumber}
