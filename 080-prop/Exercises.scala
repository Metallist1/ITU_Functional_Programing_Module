...
//> using options "-deprecation", "-feature", "-source:future"
...
object Exercise_3:
  ...
  def p1Min (minimum: List[Int] => Int): org.scalacheck.Prop = 
    org.scalacheck.Prop.forAll(intList)(ns => ns.min == minimum(ns))
...
object Exercise_4:
  ...
  trait Prop:
    ...
    infix def && (that: Prop): Prop =
      val checked = this.check
      new Prop { def check = checked && that.check}
...
object Gen:
  ...
  def choose (start: Int, stopExclusive: Int): Gen[Int] =
    State(RNG.nonNegativeLessThan(stopExclusive-start).andThen((x,y) => (x+start,y)))
  // Exercise 6
  ...
  def double: Gen[Double] = 
    State(RNG.int.andThen((x,y) => ((x.toDouble ,y))))
  ...
  // ListofN was implemented as an extension method due to the fact that we need an initialised generator to generate a list of N.
  // If we added it to the methods, we would not be able to generate values as to generate values, it requires to have an initialised Gen.
  ...
  extension [A](self: Gen[A])
    def listOf(size: Gen[Int]): Gen[List[A]] =
      size.flatMap( a =>  State.sequence(List.fill(a)(self)))
  ...
  extension [A](self: Gen[A])
    def union (that: Gen[A]): Gen[A] =
      boolean.flatMap( x => if x then self else that)
...
object Exercise_12:
  ...
  def listOfN[A: Gen](n: Int): Gen[List[A]] =
    ...
    //  State.sequence(List.fill(n)[A])
  
  def listOfN_[A: Gen]: Int => Gen[List[A]] = (n: Int) =>
    summon[Gen[A]].listOfN(n)
  ...
  def listOf[A: Gen](using genInt: Gen[Int]): Gen[List[A]] =
    genInt.flatMap( a =>  summon[Gen[A]].listOfN(a))
  
  def listOf_[A] (using genInt: Gen[Int], genA: Gen[A]): Gen[List[A]] =
    genInt.flatMap( a =>  genA.listOfN(a))
...
def forAll[A](as: Gen[A])(f: A => Boolean): Prop =
  (max, n, rng) => randomLazyList(as)(rng).zip(LazyList.from(0)).take(n).map {
    case (a, i) =>
...
extension (self: Prop)
  ...
  infix def || (that: Prop): Prop = (maxSize, tcs, rng) => 
    self(maxSize, tcs, rng) match
      case Falsified(_, _) => that(maxSize, tcs, rng)
      case x => x
...
extension [A](self: Gen[A]) 
  def list: SGen[List[A]] =
    a => State.sequence(List.fill(a)(self)) 
...
object Exercise_16:
  ...
  def p1Min(minimum: List[Int] => Int): Prop = 
    ...
    SGen.Prop.forAll(nonEmptyList)(ns => ns.min == minimum(ns))
