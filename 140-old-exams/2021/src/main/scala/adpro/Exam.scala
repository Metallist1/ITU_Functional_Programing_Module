...
object Q1 {
   ...
   *
   * abstract class Printable         { def hello (): String = "printable" }
  ...
  /* Notice that the trait Printable is not sealed.
   *
   * Q1. (10 %)
   *
   * Write a function `hello` that uses pattern matching and achieves
   * exactly the same effect as calling the method hello in Java
...
object Q7 {
  ...
  /* Q7. (5%)
   *
   * What is the problem with writing size (s) >= 10 to check whether the stream
...
object Q9 {
  ...
  class MySpec
    ...
    "Write the test here by replacing 'fail' below" in {
